...
from numba.np.random.distributions import random_wald
...
feature_names = [
    "mean", "median", "std", "var", "min", "max", "range", "skewness", "kurtosis", "rms"
]


# Function for extracting features from the 5-second segments
def rms(data):
...
with h5py.File('./hdf5_data.h5', 'w') as hdf:
    ...
    for name, i in datasets.items():
        ...
        plt.title('Y Acceleration with Moving Average', fontsize=15)
        plt.xlabel('Time (s)', fontsize=12)
        plt.ylabel('Acceleration (m/s^2)', fontsize=12)
        plt.legend()
        plt.grid(True, alpha=0.7)
        ...
        plt.title('Z Acceleration with Moving Average', fontsize=15)
        plt.xlabel('Time (s)', fontsize=12)
        plt.ylabel('Acceleration (m/s^2)', fontsize=12)
        plt.legend()
        plt.grid(True, alpha=0.7)
...
# Load from HDF5 and print features
with h5py.File('hdf5_data.h5', 'r') as hdf:
    for split in ['Train', 'Test']:
        ...
        for name in group:
            ...
            print("X-axis Feature Matrix (first 5 rows):")
            print(x_df.head(), "\n")

            print("Y-axis Feature Matrix (first 5 rows):")
            print(y_df.head(), "\n")

            print("Z-axis Feature Matrix (first 5 rows):")
            print(z_df.head(), "\n")
